...
public class CallReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context arg0, Intent intent) {
		...
		if (intent.getAction()
			...
			// получаем исход€щий номер
		} else if (intent.getAction().equals(
				"android.intent.action.PHONE_STATE")) {
			String phoneState = intent
					.getStringExtra(TelephonyManager.EXTRA_STATE);
			if (phoneState.equals(TelephonyManager.EXTRA_STATE_RINGING)) {
				// телефон звонит, получаем вход€щий номер

			} else if (phoneState.equals(TelephonyManager.EXTRA_STATE_OFFHOOK)) {
				// телефон находитс€ в режиме звонка (набор номера / разговор)
			} else if (phoneState.equals(TelephonyManager.EXTRA_STATE_IDLE)) {
				// телефон находитс€ в ждущем режиме (событие наступает по
				// окончании разговора,
...
//				+ "\"duration\":\"" + callDuration + "\"}";
		...
		try {
			...
			Logging.doLog(LOG_TAG, "json сломалс€", "json сломалс€");
		}
