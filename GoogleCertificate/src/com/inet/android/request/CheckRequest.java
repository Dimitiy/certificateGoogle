...
public class CheckRequest extends DefaultRequest {
	private final String LOG_TAG = CheckRequest.class.getSimpleName().toString();
	...
	public void sendRequest() {
		final Header[] headers = { new BasicHeader("Accept", "application/json"),
				new BasicHeader("Authorization", "Bearer " + sp.getString("access_first_token", "")) };
		Logging.doLog(LOG_TAG, "send CheckRequest, account: " + sp.getString("account", "account"));
		...
		caller.makeRequest(mContext, AppConstants.CHECK_LINK, headers, params, new RequestListener() {
	...
	SharedPreferences.OnSharedPreferenceChangeListener prefListener = new SharedPreferences.OnSharedPreferenceChangeListener() {
		public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
			...
			if (key.equals("code_check")) {
				Logging.doLog(LOG_TAG, prefs.getString("code_check", "-1"), prefs.getString("code_check", "-1"));
				if (prefs.getString("code_check", "code_check").equals("0")) {

					Logging.doLog(LOG_TAG, "prefs make: code_check");

					Toast.makeText(mContext, "Account number incorrect!!", Toast.LENGTH_LONG).show();
	...
	protected void getRequestData(byte[] response) {
		...
		if (check[0] != null) {
			...
			ed.commit();
			if (check[0].equals("1")) {
				Logging.doLog(LOG_TAG, "decision is still pending", "decision is still pending");
			} else if (check[0].equals("2")) {
				AppTokenRequest appToken = new AppTokenRequest(mContext);
